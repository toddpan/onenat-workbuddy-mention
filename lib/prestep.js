...
function pluginMessage(name, text) {
    return createUserMessage({
        content: [{ type: 'text', text }],
        source: { kind: 'plugin', plugin: PLUGIN, form: 'snapshot', sections: [{ name, text }] },
    });
}
export function registerPreStep(ctx, deps, log) {
    ctx.effect(() => ctx.on('agent/pre-step', async ({ agent, messages, signal }, next) => {
        ...
        try {
            ...
            if (injected.length === decision.messages.length)
                return decision;
    ...
    }, { prepend: true }), 'onenat-workbuddy-mention: pre-step mention injection');
}
/**
 * 把提及注入为本步的附加用户消息（紧跟其来源消息之后）。
 */
export async function inject(agent, messages, deps, log) {
    const out = [];
    for (const message of messages) {
        ...
        if (!text || !containsMention(text))
            continue;
        const mentions = deps.parser.parse(text);
        if (mentions.agents.length === 0 && mentions.resources.length === 0)
            continue;
        ...
        if (mentions.agents.length > 0)
            out.push(pluginMessage('onenat-agents', renderAgentDirective(mentions.agents, deps.store)));
        if (mentions.resources.length > 0) {
            const rendered = await renderResourceBlock(mentions.resources.map((r) => r.id), deps);
            if (rendered)
                out.push(pluginMessage('onenat-resources', rendered));
        }
    }
    return out;
}
/** 子智能体指认段 */
function renderAgentDirective(mentions, store) {
    const lines = ['[OneNat WorkBuddy] 用户在本次消息中用 @ 指定了子智能体，请按下列指认派发：', ''];
...
async function renderResourceBlock(ids, deps) {
    ...
    const sections = [];
    if (composed.block)
        sections.push(composed.block);
    for (const item of resolved.preRendered)
        sections.push(item.markdown);
    if (sections.length === 0 && composed.warnings.length === 0 && resolved.missing.length === 0)
        return '';
    const lines = ['[OneNat WorkBuddy] 用户在本次消息中用 @ 指定了下列资源，本轮可以使用它们：', ''];
...
//# sourceMappingURL=prestep.js.map