...
import type { DshRef, ResolvedDshTarget, SubAgent } from './types.js'
...
export class ManageRouter {
   ...
   */
  private normalizeDshRef(raw: any): DshRef | undefined {
   ...
   * 只是挂一个不落库的合成 agent。
   */
  private async resolveProbeTarget(dshRefRaw: any, apiKey?: string): Promise<ResolvedDshTarget> {
    const dshRef = this.normalizeDshRef(dshRefRaw)
    if (!dshRef) throw new InputError('请先选择 DSH 实体绑定（dshRef 无效）')
    const probe: SubAgent = {
      id: PROBE_AGENT_ID,
      name: PROBE_AGENT_ID,
      dshRef,
    ...
    if (!target.online || !target.baseUrl) throw new InputError(target.error || '远端 DSH 节点不可达', 502)
    return target
  }
  ...
  public async dispatch(req: IncomingMessage, res: ServerResponse, prefix: string): Promise<boolean> {
    ...
    try {
      ...
      if (path === '/api/probe/mkdir' && method === 'POST') {
        ...
        if (!parent || !name) throw new InputError('缺少目录路径或名称')
