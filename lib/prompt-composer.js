...
export class PromptComposer {
    ...
    async compose(agent, ctx) {
        ...
        const allBindings = [...(agent.resources || [])];
        ...
        for (const binding of allBindings) {
            ...
            const ep = binding.ref.kind === 'mapping'
            ...
            if (binding.note)
                lines.push(`- 用途: ${binding.note}`);
            if (ep.note && ep.note !== binding.note)
                lines.push(`- 映射备注: ${ep.note}`);
            // 资源侧技能：不再内联全文，登记为"需远端自助安装后加载"（统一指引见 [资源技能安装与加载] 段）
            const skills = ep.appSkills || [];
            const mode = binding.skillMode;
            const picked = mode === 'all' ? skills : Array.isArray(mode) ? skills.filter((s) => mode.names.includes(s.name)) : [];
            if (skills.length > 0) {
                if (mode === 'none') {
                    lines.push(`- 技能清单（按需下载后先读再用）: ${skills.map((s) => s.name).join(', ')}`);
                    for (const s of skills)
                        lines.push(`  curl -s "${s.url}"`);
                }
                else {
                    if (picked.length > 0) {
                    ...
                    if (rest.length > 0)
                        lines.push(`- 其余技能（按需下载）: ${rest.map((s) => s.name).join(', ')}`);
                }
            }
            sections.push(`### ${title}\n${lines.join('\n')}`);
        ...
        if (agent.resources?.some((r) => r.credentialMode === 'self-fetch')) {
            parts.push('');
