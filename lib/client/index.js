...
function Field(props) {
    return React.createElement('label', { className: 'onm-field' }, React.createElement('span', null, props.label), props.textarea
...
function ConnectionCard(props) {
    ...
    return React.createElement('div', { className: 'onm-card' }, React.createElement('h3', null, '① ONENAT 连接与派发默认值'), React.createElement('div', { className: 'onm-grid' }, React.createElement(Field, { label: 'ONENAT 服务地址', value: baseUrl, onChange: setBaseUrl, placeholder: 'https://onenat.sooncore.com' }), React.createElement(Field, { label: 'ONENAT API Key', value: apiKey, onChange: setApiKey, placeholder: 'onk-…' }), React.createElement(Field, { label: '派发超时（毫秒）', value: timeoutMs, onChange: setTimeoutMs })), React.createElement('div', { className: 'onm-row' }, React.createElement('label', { className: 'onm-row' }, React.createElement('input', { type: 'checkbox', checked: reuse, onChange: (e) => setReuse(e.target.checked) }), React.createElement('span', null, '同一子智能体在本会话中复用远端会话（多轮续聊）'))), React.createElement('div', { className: 'onm-row' }, React.createElement(Btn, { variant: 'primary', onClick: save }, busy ? '保存中…' : '保存')), message ? React.createElement(Banner, message) : null);
}
// ---- 资源目录 ----
function ResourceCard() {
    const state = useAsync(async () => (await consoleApi.resources(true)) || {}, [], {});
    const endpoints = state.value?.endpoints || [];
    return React.createElement('div', { className: 'onm-card' }, React.createElement('h3', null, '② ONENAT 资源目录', React.createElement('span', { className: 'onm-tag' }, String(endpoints.length))), React.createElement('div', { className: 'onm-row' }, React.createElement(Btn, { onClick: state.reload }, state.loading ? '刷新中…' : '强制刷新'), React.createElement('span', { className: 'onm-muted' }, '在输入框用 @ 可以直接选中下列资源；条目里的公网入口是实时解析结果。')), state.error ? React.createElement(Banner, { kind: 'err', text: state.error }) : null, endpoints.length === 0
        ? React.createElement('div', { className: 'onm-muted' }, '暂无资源（检查 ONENAT 地址与 Key，或确认 ONENAT 客户端在线）。')
        : React.createElement('table', { className: 'onm-table' }, React.createElement('thead', null, React.createElement('tr', null, React.createElement('th', null, '@ 名称'), React.createElement('th', null, '类型'), React.createElement('th', null, '状态'), React.createElement('th', null, '实时入口'), React.createElement('th', null, '隧道 / 技能'))), React.createElement('tbody', null, endpoints.map((ep) => React.createElement('tr', { key: ep.mappingId || ep.appId }, React.createElement('td', null, '@' + (ep.appName || ep.note || ep.mappingId)), React.createElement('td', null, String(ep.kind || '').toUpperCase()), React.createElement('td', null, React.createElement('span', { className: 'onm-tag ' + (ep.online ? 'ok' : 'off') }, ep.online ? '在线' : '离线')), React.createElement('td', { className: 'onm-code' }, ep.kind === 'ssh' ? `ssh -p ${ep.port} @${ep.host}` : (ep.baseUrl || `${ep.proto}://${ep.host}:${ep.port || '?'}`)), React.createElement('td', null, (ep.tunnelName || '') + (ep.appSkills?.length ? ` · ${ep.appSkills.length} 技能` : '')))))));
}
// ---- 子智能体管理 ----
const EMPTY_FORM = {
    id: '',
...
function AgentCard(props) {
    ...
    const remove = (agent) => {
        if (form.id !== agent.id) {
            ...
            setForm((prev) => ({ ...prev, id: agent.id, name: agent.name }));
    ...
    const refOptions = form.refKind === 'app'
        ...
        : form.refKind === 'mapping'
            ? props.endpoints
            : [];
    return React.createElement('div', { className: 'onm-card' }, React.createElement('h3', null, '③ 子智能体管理', React.createElement('span', { className: 'onm-tag' }, String(props.agents.length))), React.createElement('div', { className: 'onm-muted' }, '子智能体 = ONENAT 上的一个 DSH 实体（只存稳定 ID，端口漂移免疫）。绑好后在输入框打 @ 就能指名派发。', dshEndpoints.length > 0 ? ` 已识别 ${dshEndpoints.length} 个 DSH 实体。` : ' 暂未在资源目录里识别到 DSH 实体（app.type=http-api 且带 dsh 技能）。'), message ? React.createElement(Banner, message) : null, props.agents.length === 0
        ? React.createElement('div', { className: 'onm-muted' }, '还没有子智能体，用下面的表单新建一个。')
        : React.createElement('table', { className: 'onm-table' }, React.createElement('thead', null, React.createElement('tr', null, React.createElement('th', null, '@ 名称'), React.createElement('th', null, '绑定实体'), React.createElement('th', null, '模型 / preset'), React.createElement('th', null, '工作目录'), React.createElement('th', null, '技能 / 资源'), React.createElement('th', null, '操作'))), React.createElement('tbody', null, props.agents.map((agent) => React.createElement('tr', { key: agent.id }, React.createElement('td', null, '@' + agent.name, agent.enabled === false ? React.createElement('span', { className: 'onm-tag off' }, '停用') : null), React.createElement('td', { className: 'onm-code' }, agent.dshRef?.kind === 'direct' ? agent.dshRef.apiBaseUrl : (agent.dshRef?.kind === 'app' ? agent.dshRef.appId : agent.dshRef?.mappingId)), React.createElement('td', null, `${agent.model || '远端默认'}${agent.agentPreset ? ' · ' + agent.agentPreset : ''}`), React.createElement('td', { className: 'onm-code' }, agent.workDir || '—'), React.createElement('td', null, `${(agent.skills || []).length} 技能 · ${(agent.resources || []).length} 资源`), React.createElement('td', null, React.createElement('div', { className: 'onm-row' }, React.createElement(Btn, { onClick: () => edit(agent) }, '编辑'), React.createElement(Btn, { onClick: () => ping(agent) }, '探测'), React.createElement(Btn, { onClick: () => preview(agent) }, '提示词'), React.createElement(Btn, { variant: 'danger', onClick: () => remove(agent) }, '删除'))))))), React.createElement('div', { className: 'onm-section-title' }, form.id ? `编辑：${form.name}` : '新增子智能体'), React.createElement('div', { className: 'onm-grid' }, React.createElement(Field, { label: '@ 名称（唯一、便于指名）', value: form.name, onChange: (v) => set({ name: v }), placeholder: '例如 kb-136-builder' }), React.createElement('label', { className: 'onm-field' }, React.createElement('span', null, 'DSH 实体绑定方式'), React.createElement('select', { value: form.refKind, onChange: (e) => set({ refKind: e.target.value, refValue: '' }) }, React.createElement('option', { value: 'mapping' }, 'ONENAT 映射（mappingId，推荐）'), React.createElement('option', { value: 'app' }, 'ONENAT 应用（appId）'), React.createElement('option', { value: 'direct' }, '直连 URL（兜底）'))), form.refKind === 'direct'
        ? React.createElement(Field, { label: 'API Base URL', value: form.apiBaseUrl, onChange: (v) => set({ apiBaseUrl: v }), placeholder: 'http://host:port/api/v1' })
        : React.createElement('label', { className: 'onm-field' }, React.createElement('span', null, form.refKind === 'app' ? '选择应用' : '选择映射'), React.createElement('select', { value: form.refValue, onChange: (e) => set({ refValue: e.target.value }) }, React.createElement('option', { value: '' }, '— 请选择 —'), refOptions.map((ep) => React.createElement('option', {
            key: form.refKind === 'app' ? ep.appId : ep.mappingId,
            value: form.refKind === 'app' ? ep.appId : ep.mappingId,
        }, `${ep.appName || ep.note || ep.mappingId} · ${String(ep.kind).toUpperCase()} · ${ep.online ? '在线' : '离线'}`)))), React.createElement(Field, { label: 'Agent Preset', value: form.agentPreset, onChange: (v) => set({ agentPreset: v }), placeholder: 'cordis' }), React.createElement(Field, { label: '模型（provider/model 或 model）', value: form.model, onChange: (v) => set({ model: v }) }), React.createElement(Field, { label: '运行权限', value: form.permission, onChange: (v) => set({ permission: v }), placeholder: 'danger-full-access / workspace-write / read-only' }), React.createElement(Field, { label: '远端工作目录（绝对路径）', value: form.workDir, onChange: (v) => set({ workDir: v }) }), React.createElement(Field, { label: '远端已装技能（逗号分隔，派发时写 /名 手势）', value: form.skills, onChange: (v) => set({ skills: v }) }), React.createElement(Field, { label: '一句话说明（@ 菜单副标题）', value: form.description, onChange: (v) => set({ description: v }) })), React.createElement('div', { className: 'onm-field', style: { marginTop: 8 } }, React.createElement('span', null, '角色提示词（systemPrompt，派发时置于任务之前）'), React.createElement('textarea', { value: form.systemPrompt, onChange: (e) => set({ systemPrompt: e.target.value }) })), React.createElement('div', { className: 'onm-row', style: { marginTop: 10 } }, React.createElement(Btn, { variant: 'primary', onClick: save }, busy ? '保存中…' : (form.id ? '更新子智能体' : '新增子智能体')), form.id ? React.createElement(Btn, { onClick: () => setForm({ ...EMPTY_FORM }) }, '取消编辑') : null, React.createElement('label', { className: 'onm-row' }, React.createElement('input', { type: 'checkbox', checked: form.enabled, onChange: (e) => set({ enabled: e.target.checked }) }), React.createElement('span', null, '启用'))), detail ? React.createElement('pre', { className: 'onm-pre onm-code' }, detail) : null);
}
// ---- 本地 SSH 资源池 ----
function SshCard(props) {
    const state = useAsync(async () => (await consoleApi.sshList()) || [], [], []);
    const [form, setForm] = React.useState({ id: '', name: '', host: '', port: '22', authType: 'password', username: 'root', password: '', privateKey: '', description: '' });
    const [message, setMessage] = React.useState(undefined);
    const [detail, setDetail] = React.useState(undefined);
    ...
    return React.createElement('div', { className: 'onm-card' }, React.createElement('h3', null, '④ 本地 SSH 资源池', React.createElement('span', { className: 'onm-tag' }, String(resources.length))), React.createElement('div', { className: 'onm-muted' }, '补充 ONENAT 之外的直连主机。@ 菜单里选中后，凭证会随资源清单一并交给执行方。'), message ? React.createElement(Banner, message) : null, resources.length === 0
        ? React.createElement('div', { className: 'onm-muted' }, '暂无本地 SSH 资源。')
        : React.createElement('div', { className: 'onm-list' }, resources.map((r) => React.createElement('div', { className: 'onm-list-item', key: r.id }, React.createElement('span', null, `${r.name} · ${r.username}@${r.host}:${r.port}${r.lastTestOk === undefined ? '' : (r.lastTestOk ? ' · 上次连通 ✓' : ' · 上次失败 ✗')}`), React.createElement('div', { className: 'onm-row' }, React.createElement(Btn, { onClick: () => { setDetail('测试中…'); consoleApi.sshTest(r.id).then((d) => setDetail(JSON.stringify(d, null, 2))).catch((e) => setDetail(String(e?.message || e))); } }, '测试'), React.createElement(Btn, { onClick: () => setForm({ id: r.id, name: r.name, host: r.host, port: String(r.port), authType: r.authType, username: r.username, password: '', privateKey: '', description: r.description || '' }) }, '编辑'), React.createElement(Btn, { variant: 'danger', onClick: () => { consoleApi.sshDelete(r.id).then(() => { state.reload(); props.onChanged(); }); } }, '删除'))))), React.createElement('div', { className: 'onm-section-title' }, form.id ? `编辑：${form.name}` : '新增 SSH 资源'), React.createElement('div', { className: 'onm-grid' }, React.createElement(Field, { label: '名称（@ 菜单显示）', value: form.name, onChange: (v) => set({ name: v }) }), React.createElement(Field, { label: '主机 IP / 域名', value: form.host, onChange: (v) => set({ host: v }) }), React.createElement(Field, { label: '端口', value: form.port, onChange: (v) => set({ port: v }) }), React.createElement(Field, { label: '登录账号', value: form.username, onChange: (v) => set({ username: v }) }), React.createElement('label', { className: 'onm-field' }, React.createElement('span', null, '认证方式'), React.createElement('select', { value: form.authType, onChange: (e) => set({ authType: e.target.value }) }, React.createElement('option', { value: 'password' }, '密码'), React.createElement('option', { value: 'key' }, '私钥'))), React.createElement(Field, { label: '说明', value: form.description, onChange: (v) => set({ description: v }) })), form.authType === 'password'
        ? React.createElement(Field, { label: '密码（留空表示不修改）', value: form.password, onChange: (v) => set({ password: v }) })
        : React.createElement('div', { className: 'onm-field', style: { marginTop: 8 } }, React.createElement('span', null, '私钥（PEM，留空表示不修改）'), React.createElement('textarea', { value: form.privateKey, onChange: (e) => set({ privateKey: e.target.value }) })), React.createElement('div', { className: 'onm-row', style: { marginTop: 10 } }, React.createElement(Btn, { variant: 'primary', onClick: save }, form.id ? '更新' : '新增'), form.id ? React.createElement(Btn, { onClick: () => setForm({ id: '', name: '', host: '', port: '22', authType: 'password', username: 'root', password: '', privateKey: '', description: '' }) }, '取消编辑') : null), detail ? React.createElement('pre', { className: 'onm-pre onm-code' }, detail) : null);
}
// ---- 输入框下方的 @ 状态指示器 ----
/**
...
function MentionStatusLine() {
    ...
    return React.createElement('div', { className: 'onm-statusline' }, React.createElement('span', { className: 'onm-statusdot', style: { background: tone } }), React.createElement('span', null, label), React.createElement('button', { type: 'button', className: 'onm-btn', style: { padding: '0 8px', fontSize: '11px' }, onClick: refresh }, busy ? '刷新中…' : '刷新'));
}
// ---- 页面 ----
function MentionTester() {
    const [text, setText] = React.useState('请 @kb-136-builder 帮我检查一下环境');
    const [result, setResult] = React.useState(undefined);
    return React.createElement('div', { className: 'onm-card' }, React.createElement('h3', null, '⑤ 提及解析自检'), React.createElement('div', { className: 'onm-muted' }, '验证一段文本会被解析成哪些实体（用于确认 @ 名称是否唯一可辨）。'), React.createElement('div', { className: 'onm-row' }, React.createElement('input', { style: { flex: 1, padding: '5px 8px', borderRadius: 6, border: '1px solid var(--dsw-alias-border-secondary, #cbd5e1)', font: 'inherit', background: 'var(--dsw-alias-bg-primary, #fff)', color: 'inherit' }, value: text, onChange: (e) => setText(e.target.value) }), React.createElement(Btn, { onClick: () => consoleApi.parse(text).then((d) => setResult(JSON.stringify(d, null, 2))).catch((e) => setResult(String(e?.message || e))) }, '解析')), result ? React.createElement('pre', { className: 'onm-pre onm-code' }, result) : null);
}
function Section(props) {
    ...
    return React.createElement('div', { className: 'onm-root' }, React.createElement('div', { className: 'onm-muted' }, 'OneNat WorkBuddy @ —— 在 DSH 原生输入框用 @ 指定 ONENAT 上的子智能体与资源。', '选中即插入胶囊，发送后由宿主把「指认指令」与「资源清单（实时入口 + 凭证策略）」注入本轮上下文；', '子智能体派发走 onenat_agent 工具，远端会话按本会话长持复用。'), agents.error ? React.createElement(Banner, { kind: 'err', text: agents.error }) : null, React.createElement(ConnectionCard, { settings: settings.value, onSaved: refreshAll }), React.createElement(ResourceCard, null), React.createElement(AgentCard, { agents: agentList, endpoints, onChanged: refreshAll }), React.createElement(SshCard, { onChanged: refreshAll }), React.createElement(MentionTester, null));
}
// ---------------------------------------------------------------------- apply
