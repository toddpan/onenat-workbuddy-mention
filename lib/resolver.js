...
export class AgentResolver {
    ...
    async resolve(agent) {
        ...
        if (agent.dshRef.kind === 'direct') {
            ...
            if (!ep)
                return { ...base, error: `映射 ${agent.dshRef.mappingId} 已不存在（请在子智能体里重新绑定 DSH 实体）` };
            ...
            if (!ep)
                return { ...base, error: `应用 ${agent.dshRef.appId} 未绑定任何映射或已删除` };
            if (!ep.online || !ep.baseUrl) {
        ...
        if (!apiKey && credMappingId) {
            ...
            if (cred.ok)
                apiKey = cred.apiKey || cred.token || undefined;
