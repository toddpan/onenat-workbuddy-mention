...
import {
  MENTION_SCHEME_AGENT,
  MENTION_SCHEME_RESOURCE,
  SSH_RESOURCE_PREFIX,
} from './types.js'
import type { ExtractedMentions, MentionCandidate, ParsedMention, SubAgent } from './types.js'
/** 从 UUID / 本地 id 生成稳定 URI */
export function agentUri(agentId: string): string {
  return MENTION_SCHEME_AGENT + agentId
}

export function mappingUri(mappingId: string): string {
  return MENTION_SCHEME_RESOURCE + mappingId
}

export function sshUri(sshResourceId: string): string {
  return MENTION_SCHEME_RESOURCE + SSH_RESOURCE_PREFIX + sshResourceId
}

/** URI → kind/id；非法 URI 返回 undefined */
export function decodeMentionUri(uri: string): { kind: 'agent' | 'resource'; id: string } | undefined {
...
/** 模型可见 / 剪贴板文本：`@[名称](uri)` */
export function mentionText(label: string, uri: string): string {
...
export class MentionParser {
  ...
  public candidates(query = ''): MentionCandidate[] {
    const q = String(query || '').trim().toLowerCase()
    const agents: MentionCandidate[] = []
    const resources: MentionCandidate[] = []
  ...
  public parse(text: string): ExtractedMentions {
    const agents: ParsedMention[] = []
    const resources: ParsedMention[] = []
    const seen = new Set<string>()
    const scan = stripCode(text)

    const pushResolved = (kind: 'agent' | 'resource', id: string, label: string, raw: string, uri: string): void => {
      const dedupeKey = kind + ':' + id
      if (seen.has(dedupeKey)) return
      seen.add(dedupeKey)
      const mention: ParsedMention = { kind, uri, label: label || id, raw, id }
      if (kind === 'agent') agents.push(mention)
      else resources.push(mention)
    }
    ...
    for (const m of scan.matchAll(BARE_URI)) {
      ...
      if (handledRanges.some(([s, e]) => start >= s && start < e)) continue
      const uri = m[1]!
      const decoded = decodeMentionUri(uri)
      if (!decoded) continue
      const resolved = this.resolveOne(decoded.kind, decoded.id)
  ...
  public resolveOne(kind: 'agent' | 'resource', id: string): { kind: 'agent' | 'resource'; id: string; name: string; uri: string } | undefined {
    if (kind === 'agent') {
  ...
  private exactIndex(): Map<string, { kind: 'agent' | 'resource'; id: string; name: string; uri: string }> {
    const map = new Map<string, { kind: 'agent' | 'resource'; id: string; name: string; uri: string }>()
    const put = (key: string, value: { kind: 'agent' | 'resource'; id: string; name: string; uri: string }): void => {
      const k = key.toLowerCase()
      if (k && !map.has(k)) map.set(k, value)
    }
    for (const agent of this.store.getAgents()) {
      if (agent.enabled === false) continue
      const value = { kind: 'agent' as const, id: agent.id, name: agent.name, uri: agentUri(agent.id) }
    ...
    for (const ep of this.directory.listEndpoints()) {
      ...
      if (!id) continue
      const value = { kind: 'resource' as const, id, name: ep.appName || ep.note || id, uri: mappingUri(id) }
      if (ep.appName) put(ep.appName, value)
      if (ep.note) put(ep.note, value)
      put(ep.mappingId, value)
      if (ep.appId) put(ep.appId, value)
      if (ep.appName && ep.tunnelName) put(`${ep.appName} (${ep.tunnelName})`, value)
    }
    for (const ssh of this.sshStore.list()) {
      const value = { kind: 'resource' as const, id: `ssh:${ssh.id}`, name: ssh.name, uri: sshUri(ssh.id) }
      put(ssh.name, value)
      put(ssh.id, value)
    }
    return map
...
function describeDshRef(agent: SubAgent): string {
  if (agent.dshRef.kind === 'mapping') return 'DSH 实体（映射绑定）'
  if (agent.dshRef.kind === 'app') return 'DSH 实体（应用绑定）'
  return `直连 ${agent.dshRef.apiBaseUrl || '未配置'}`
}

/** 模糊打分：命中越靠前、越短、越连续，分越高 */
function score(row: MentionCandidate, q: string): number {
  const name = row.name.toLowerCase()
  const desc = row.description.toLowerCase()
  if (name === q) return 1000
  if (name.startsWith(q)) return 800 - name.length
  const idx = name.indexOf(q)
  if (idx >= 0) return 600 - idx - name.length
  let s = 0
  let cursor = 0
  for (const ch of q) {
