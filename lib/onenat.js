...
function parsePublicUrl(publicUrl) {
    if (!publicUrl)
        return undefined;
...
function looksLikeWebPort(local) {
...
function classifyKind(app, local, proto) {
    const type = (app?.type || '').toLowerCase();
    if (type === 'ssh' || /:(\d+)$/.exec(local)?.[1] === '22')
        return 'ssh';
    if (type === 'http-api' && app?.skills?.some((s) => /dsh/i.test(s.name)))
        return 'dsh';
    if (type === 'http-api' || proto === 'http' || looksLikeWebPort(local))
        return 'http';
    return 'tcp';
}
export class OnenatDirectory {
    ...
    async refresh(force = false) {
        if (!this.configured)
            throw new Error('ONENAT 未配置（baseUrl / apiKey 缺失）');
     ...
     */
    async fetchMappingCredentials(mappingId, force = false) {
        const cached = this.credCache.get(mappingId);
        if (!force && cached && Date.now() - cached.at < OnenatDirectory.CRED_TTL)
            return cached.data;
        try {
            ...
            if (!res.ok) {
                const data = { ok: false, error: json?.error || `HTTP ${res.status}` };
    ...
    static async pingDsh(baseUrl, apiKey) {
        try {
...
//# sourceMappingURL=onenat.js.map