...
`;
		...
		*/
		const consoleApi = {
			...
			probeOptions: (body) => api("/api/probe/options", {
				method: "POST",
				body
			}),
			probeSkills: (body) => api("/api/probe/skills", {
				method: "POST",
				body
			}),
			probeFs: (body) => api("/api/probe/fs", {
				method: "POST",
				body
			}),
			probeMkdir: (body) => api("/api/probe/mkdir", {
				method: "POST",
				body
			}),
		...
		function Field(props) {
			return react.default.createElement("div", { className: "onm-field" }, props.label ? react.default.createElement("span", { className: "onm-label" }, props.label) : null, props.children, props.hint ? react.default.createElement("span", { className: "onm-hint" }, props.hint) : null);
		}
		...
		function TextArea(props) {
			return react.default.createElement("textarea", {
		...
		function Select(props) {
			return react.default.createElement("select", {
				...
				disabled: props.disabled,
				onChange: (e) => props.onChange(e.target.value)
			}, props.options.map((o) => react.default.createElement("option", {
				key: o.value,
				value: o.value,
				title: o.title,
				disabled: o.disabled
		...
		function Tag(props) {
			return react.default.createElement("span", {
				className: "onm-tag" + (props.tone ? " " + props.tone : ""),
				title: props.title
			}, props.children);
		}
		...
		function MetaRow(props) {
			return [react.default.createElement("dt", { key: "k" }, props.label), react.default.createElement("dd", { key: "v" }, props.children)];
		}
		...
		function Overlay(props) {
			...
			}, react.default.createElement("div", { className: "onm-modal-head" }, react.default.createElement("h3", { className: "onm-modal-title" }, props.title), props.subtitle ? react.default.createElement("span", { className: "onm-muted" }, props.subtitle) : null, react.default.createElement("div", { className: "onm-spacer" }), react.default.createElement(Btn, {
				variant: "ghost",
				size: "sm",
				onClick: props.onClose,
				title: "关闭"
			}, "✕")), react.default.createElement("div", { className: "onm-modal-body" }, props.children), props.footer ? react.default.createElement("div", { className: "onm-modal-foot" }, props.footer) : null)), document.body);
		}
		/**
		...
		*/
		function DirPicker(props) {
			...
			const [typed, setTyped] = react.default.useState(props.initial || "");
			const [error, setError] = react.default.useState(void 0);
			const [showHidden, setShowHidden] = react.default.useState(false);
			const [creating, setCreating] = react.default.useState(void 0);
			...
			react.default.useEffect(() => {
				load(props.initial || void 0);
			}, []);
			const mkdir = () => {
				const name = String(creating || "").trim();
				if (!name) return;
				setBusy(true);
				consoleApi.probeMkdir({
					dshRef: props.dshRef,
					apiKey: props.apiKey,
					path: state.path,
					name
			...
			const rows = [];
			if (state.parent) rows.push(react.default.createElement("div", {
				key: "up",
				className: "onm-dirrow up",
				onClick: () => load(state.parent)
			}, react.default.createElement("span", null, "↩"), react.default.createElement("span", { className: "nm" }, "上级目录")));
			for (const e of entries) rows.push(react.default.createElement("div", {
				key: e.path,
				className: "onm-dirrow" + (e.hidden ? " is-hidden" : ""),
				title: e.path,
				onClick: () => load(e.path)
			}, react.default.createElement("span", null, "📁"), react.default.createElement("span", { className: "nm" }, e.name), react.default.createElement("span", { className: "chev" }, "›")));
			if (!state.loading && rows.length === 0 && !error) rows.push(react.default.createElement("div", {
				key: "empty",
				className: "onm-empty"
			}, "此目录下没有子目录"));
			if (state.loading) rows.push(react.default.createElement("div", {
				key: "loading",
				className: "onm-empty"
			}, "加载中…"));
			return react.default.createElement(Overlay, {
				title: "选择远端工作目录",
				subtitle: props.subtitle,
				onClose: props.onClose,
				top: true,
				footer: [react.default.createElement(Btn, {
					key: "cancel",
					onClick: props.onClose
				}, "取消"), react.default.createElement(Btn, {
					key: "pick",
					variant: "primary",
					disabled: !state.path,
					onClick: () => props.onPick(state.path)
				}, "使用此目录")]
			}, react.default.createElement("div", { className: "onm-db-path" }, react.default.createElement(Btn, {
				size: "sm",
				title: "回到远端主目录",
				onClick: () => load()
			}, "🏠"), react.default.createElement(TextInput, {
				value: typed,
				mono: true,
				...
				size: "sm",
				onClick: () => typed.trim() && load(typed.trim())
			}, "前往")), error ? react.default.createElement(Banner, {
				kind: "err",
				text: "浏览失败：" + error
			}) : null, react.default.createElement("div", { className: "onm-db-list" }, rows), react.default.createElement("div", { className: "onm-row" }, react.default.createElement(Btn, {
				size: "sm",
				disabled: !state.path,
				onClick: () => setCreating(creating === void 0 ? "" : void 0)
			}, "＋ 新建文件夹"), react.default.createElement(Btn, {
				size: "sm",
				variant: "ghost",
				onClick: () => setShowHidden(!showHidden)
			}, showHidden ? "✓ 显示隐藏目录" : "显示隐藏目录"), react.default.createElement("div", { className: "onm-spacer" }), state.truncated ? react.default.createElement("span", { className: "onm-hint" }, "目录过多，仅显示开头部分") : null), creating !== void 0 ? react.default.createElement("div", { className: "onm-row" }, react.default.createElement(TextInput, {
				value: creating,
				...
				onClick: mkdir
			}, busy ? "创建中…" : "创建")) : null, react.default.createElement("div", { className: "onm-hint" }, state.path ? `当前：${state.path}` : "尚未进入任何目录"));
		}
		const EMPTY_FORM = {
			id: "",
			name: "",
		...
		const PERMISSIONS = [
			...
			},
			{
				...
				label: "workspace-write · 工作区可写"
			},
			{
				value: "danger-full-access",
				label: "danger-full-access · 完全访问"
			}
		];
		...
		function dshRefOf(form) {
			...
			if (!form.refValue) return void 0;
			return form.refKind === "app" ? {
				kind: "app",
				appId: form.refValue
			} : {
				kind: "mapping",
		...
		function AgentEditor(props) {
			...
			const [form, setForm] = react.default.useState(() => formFromAgent(props.agent));
			const [remote, setRemote] = react.default.useState(void 0);
			const [sync, setSync] = react.default.useState({
				state: "idle",
				text: ""
			});
			const [message, setMessage] = react.default.useState(void 0);
			const [busy, setBusy] = react.default.useState(false);
			const [picker, setPicker] = react.default.useState(false);
			const [permCustom, setPermCustom] = react.default.useState(() => Boolean(form.permission) && !PERMISSIONS.some((p) => p.value === form.permission));
			const [skillOptions, setSkillOptions] = react.default.useState(void 0);
			const [skillBusy, setSkillBusy] = react.default.useState(false);
			...
			const syncRemote = () => {
				...
				setSync({
					state: "loading",
					text: "正在从远端 DSH 拉取模型与模式预设…"
				});
				consoleApi.probeOptions({
					dshRef: target,
					...
					setSync({
						state: "error",
						text: "远端选项同步失败：" + String(err?.message || err) + " —— 可手工填写"
					});
				});
			};
			react.default.useEffect(() => {
			...
			const firstBind = react.default.useRef(true);
			react.default.useEffect(() => {
			...
			const loadSkills = () => {
				...
				setSkillBusy(true);
				consoleApi.probeSkills({
					dshRef: target,
					apiKey: form.apiKey || void 0,
					cwd: form.workDir || void 0
				}).then((data) => {
					const skills = Array.isArray(data?.skills) ? data.skills : [];
					setSkillOptions(skills);
					setMessage(skills.length > 0 ? {
						kind: "info",
						text: `远端返回 ${skills.length} 个已装技能，点选即可加入清单。`
					} : {
						kind: "info",
						text: "远端没有返回技能（可能未安装技能或服务端版本较低）。"
					});
				}).catch((err) => setMessage({
					kind: "err",
					text: "技能清单拉取失败：" + String(err?.message || err)
				})).finally(() => setSkillBusy(false));
			};
			const skills = String(form.skills || "").split(",").map((s) => s.trim()).filter(Boolean);
			const toggleSkill = (name) => {
				set({ skills: (skills.includes(name) ? skills.filter((s) => s !== name) : [...skills, name]).join(", ") });
			};
			const save = () => {
				...
				if (!target) {
					setMessage({
						kind: "err",
						text: "请选择绑定的 DSH 实体（ONENAT 映射 / 应用）或填写直连地址"
					});
					return;
				}
				setBusy(true);
				...
				}).then(() => props.onSaved(isEdit ? `已更新「${form.name}」` : `已新增「${form.name}」`)).catch((err) => setMessage({
					kind: "err",
			...
			const presetOptions = [];
			if (!form.agentPreset) presetOptions.push({
				value: "",
				label: "远端默认"
			});
			for (const p of presets) presetOptions.push({
				value: p.id,
				label: p.name && p.name !== p.id ? `${p.id} · ${p.name}` : p.id,
				title: p.description
			});
			if (form.agentPreset && !presets.some((p) => p.id === form.agentPreset)) presetOptions.unshift({
				value: form.agentPreset,
				label: `${form.agentPreset}（当前）`
			});
			...
			if (form.provider && !providers.includes(form.provider)) providerOptions.unshift({
				value: form.provider,
				label: `${form.provider}（当前）`
			});
			...
			for (const m of visibleModels) modelOptions.push({
				value: m.id,
			...
			if (form.model && !visibleModels.some((m) => m.id === form.model)) modelOptions.unshift({
				value: form.model,
				label: `${form.model}（当前）`
			});
			return react.default.createElement(Overlay, {
				title: isEdit ? `编辑子智能体：@${form.name || ""}` : "新增子智能体",
				subtitle: remote?.baseUrl ? `远端 ${remote.baseUrl}` : void 0,
				onClose: props.onClose,
				footer: [
					message ? react.default.createElement("span", {
						key: "msg",
						className: "onm-hint"
					}, message.text) : null,
					react.default.createElement("div", {
						key: "sp",
						className: "onm-spacer"
					}),
					react.default.createElement(Btn, {
						key: "cancel",
						onClick: props.onClose
			...
			}, message ? react.default.createElement(Banner, {
				kind: message.kind,
				text: message.text
			}) : null, react.default.createElement("div", { className: "onm-grid" }, react.default.createElement(Field, {
				label: "@ 名称（唯一、便于指名）",
				children: react.default.createElement(TextInput, {
					value: form.name,
				...
				label: "DSH 实体绑定方式",
				children: react.default.createElement(Select, {
					value: form.refKind,
					onChange: (v) => set({
						refKind: v,
						refValue: ""
					...
					options: [
						{
							value: "mapping",
							label: "ONENAT 映射（mappingId，推荐）"
						},
						{
							value: "app",
							label: "ONENAT 应用（appId）"
						},
						{
							value: "direct",
							label: "直连 URL（兜底）"
			...
			})), form.refKind === "direct" ? react.default.createElement(Field, {
				label: "API Base URL",
				...
				children: react.default.createElement(TextInput, {
					...
					placeholder: "留空则经 ONENAT 映射凭证接口解析"
				})
			}), react.default.createElement("div", { className: "onm-sub" }, "远端运行参数（模式 / 模型 / 工作目录）"), react.default.createElement("div", { className: "onm-row" }, react.default.createElement(Btn, {
				size: "sm",
				disabled: !canProbe || sync.state === "loading",
				onClick: syncRemote
			}, sync.state === "loading" ? "同步中…" : "↻ 从远端 DSH 拉取"), react.default.createElement("span", { className: "onm-hint" }, sync.text || "模式预设、Provider、模型清单与目录，都取自该 DSH 实体当前实际可用的东西。")), react.default.createElement("div", { className: "onm-grid" }, react.default.createElement(Field, {
				label: "模式（agentPreset）",
				hint: presets.length > 0 ? "来自远端 DSH 的 agent preset 清单。" : "远端清单不可用时手工填写。",
				children: presets.length > 0 ? react.default.createElement(Select, {
					value: form.agentPreset,
				...
				children: providers.length > 0 ? react.default.createElement(Select, {
					value: form.provider,
					onChange: (v) => set({
						provider: v,
						model: ""
				...
				children: models.length > 0 ? react.default.createElement(Select, {
					value: form.model,
			...
			})), permCustom ? react.default.createElement(Field, {
				label: "自定义运行权限",
				children: react.default.createElement(TextInput, {
					value: form.permission,
					onChange: (v) => set({ permission: v }),
					placeholder: "例如 danger-full-access"
				})
			}) : null, react.default.createElement(Field, {
				label: "工作目录（远端绝对路径）",
				hint: "该成员远端会话的 cwd：文件工具根目录、附件落盘处；留空用远端默认。",
				children: react.default.createElement("div", { className: "onm-row" }, react.default.createElement("div", { style: {
					flex: "1 1 200px",
					minWidth: 0
				} }, react.default.createElement(TextInput, {
					value: form.workDir,
					...
					placeholder: "如 /data/panzj/workspace/demo"
				})), react.default.createElement(Btn, {
					disabled: !canProbe,
					title: canProbe ? "浏览远端目录" : "先选择 DSH 实体",
					onClick: () => setPicker(true)
				}, "📁 浏览远端…"))
			}), react.default.createElement("div", { className: "onm-sub" }, "技能与提示词"), react.default.createElement(Field, {
				label: "远端已装技能（逗号分隔，派发时写 /名 手势）",
				hint: "点下方按钮从远端拉当前已装技能清单，点选即可加入。",
				children: react.default.createElement(TextInput, {
					value: form.skills,
					onChange: (v) => set({ skills: v }),
					placeholder: "baidu-netdisk, kb-bugs"
				})
			}), react.default.createElement("div", { className: "onm-row" }, react.default.createElement(Btn, {
				size: "sm",
				disabled: !canProbe || skillBusy,
				onClick: loadSkills
			}, skillBusy ? "拉取中…" : "↻ 拉取远端技能清单"), skillOptions && skillOptions.length > 0 ? react.default.createElement("span", { className: "onm-hint" }, `远端 ${skillOptions.length} 个技能，点选切换`) : null), skillOptions && skillOptions.length > 0 ? react.default.createElement("div", { className: "onm-chips" }, skillOptions.map((s) => react.default.createElement("span", {
				key: s.name,
				className: "onm-chip" + (skills.includes(s.name) ? " on" : ""),
				title: s.description || s.name,
				onClick: () => toggleSkill(s.name)
			}, s.name))) : null, react.default.createElement(Field, {
				label: "一句话说明（@ 菜单副标题）",
				children: react.default.createElement(TextInput, {
					value: form.description,
					onChange: (v) => set({ description: v }),
					placeholder: "例如 负责 KB 后端构建与回归"
				})
			}), react.default.createElement(Field, {
				label: "角色提示词（systemPrompt，派发时置于任务之前）",
				children: react.default.createElement(TextArea, {
					value: form.systemPrompt,
					onChange: (v) => set({ systemPrompt: v }),
					placeholder: "你是……负责……"
				})
			}), react.default.createElement("label", { className: "onm-row" }, react.default.createElement("input", {
				type: "checkbox",
				checked: form.enabled,
				onChange: (e) => set({ enabled: e.target.checked })
			}), react.default.createElement("span", null, "启用（停用后不出现在 @ 菜单）")), picker && ref ? react.default.createElement(DirPicker, {
				dshRef: ref,
				apiKey: form.apiKey || void 0,
				subtitle: form.name || void 0,
				initial: form.workDir || void 0,
				onPick: (path) => {
					set({ workDir: path });
					setPicker(false);
				},
		...
		function AgentCard(props) {
			const [editing, setEditing] = react.default.useState(void 0);
			const [message, setMessage] = react.default.useState(void 0);
			const [detail, setDetail] = react.default.useState(void 0);
			const [confirmId, setConfirmId] = react.default.useState(void 0);
			const ping = (agent) => {
			...
			return react.default.createElement("div", { className: "onm-card" }, react.default.createElement("div", { className: "onm-card-head" }, react.default.createElement("h3", { className: "onm-card-title" }, "子智能体", react.default.createElement(Tag, { tone: "brand" }, String(props.agents.length))), react.default.createElement("div", { className: "onm-spacer" }), react.default.createElement(Btn, {
				variant: "primary",
				size: "sm",
				onClick: () => setEditing(null)
			}, "＋ 新增")), react.default.createElement("div", { className: "onm-muted" }, "子智能体 = ONENAT 上的一个 DSH 实体（只存稳定 ID，端口漂移免疫）。绑好后在输入框打 @ 就能指名派发。"), message ? react.default.createElement("div", { style: { marginTop: 8 } }, react.default.createElement(Banner, {
				kind: message.kind,
				text: message.text
			})) : null, props.agents.length === 0 ? react.default.createElement("div", {
				className: "onm-muted",
				style: { marginTop: 8 }
			}, "还没有子智能体，点右上角「新增」创建一个。") : react.default.createElement("div", { className: "onm-list" }, props.agents.map((agent) => react.default.createElement("div", {
				key: agent.id,
				className: "onm-item"
			}, react.default.createElement("div", { className: "onm-item-head" }, react.default.createElement("span", { className: "onm-item-title" }, "@" + agent.name), agent.enabled === false ? react.default.createElement(Tag, { tone: "off" }, "已停用") : react.default.createElement(Tag, { tone: "ok" }, "启用"), react.default.createElement("div", { className: "onm-spacer" }), react.default.createElement(Tag, null, agent.model ? `${agent.provider ? agent.provider + " / " : ""}${agent.model}` : "远端默认模型")), react.default.createElement("dl", { className: "onm-meta" }, react.default.createElement(MetaRow, {
				label: "绑定",
				children: react.default.createElement("span", { className: "onm-mono" }, refLabel(agent))
			}), react.default.createElement(MetaRow, {
				label: "模式",
				children: agent.agentPreset || "cordis"
			}), react.default.createElement(MetaRow, {
				label: "工作目录",
				children: react.default.createElement("span", { className: "onm-mono" }, agent.workDir || "远端默认")
			}), react.default.createElement(MetaRow, {
				label: "技能/资源",
				children: `${(agent.skills || []).length} 技能 · ${(agent.resources || []).length} 资源`
			})), react.default.createElement("div", { className: "onm-actions" }, react.default.createElement(Btn, {
				size: "sm",
				onClick: () => setEditing(agent)
			}, "编辑"), react.default.createElement(Btn, {
				size: "sm",
				onClick: () => ping(agent)
			}, "探测"), react.default.createElement(Btn, {
				size: "sm",
				onClick: () => preview(agent)
			}, "提示词"), react.default.createElement(Btn, {
				size: "sm",
				variant: "danger",
				onClick: () => remove(agent)
			}, confirmId === agent.id ? "确认删除" : "删除"))))), detail ? react.default.createElement("div", null, react.default.createElement("div", {
				className: "onm-row",
				style: { marginTop: 10 }
			}, react.default.createElement("span", {
				className: "onm-sub",
				style: { margin: 0 }
			}, "探测详情"), react.default.createElement("div", { className: "onm-spacer" }), react.default.createElement(Btn, {
				size: "sm",
				variant: "ghost",
				onClick: () => setDetail(void 0)
			}, "收起")), react.default.createElement("pre", { className: "onm-pre onm-mono" }, detail)) : null, editing !== void 0 ? react.default.createElement(AgentEditor, {
				agent: editing,
		...
		function ConnectionCard(props) {
			...
			return react.default.createElement("div", { className: "onm-card" }, react.default.createElement("div", { className: "onm-card-head" }, react.default.createElement("h3", { className: "onm-card-title" }, "ONENAT 连接与派发默认值"), react.default.createElement("div", { className: "onm-spacer" }), react.default.createElement(Btn, {
				size: "sm",
				variant: "ghost",
				onClick: () => setOpen(!open)
			}, open ? "收起" : "展开")), open ? react.default.createElement("div", { style: {
				display: "flex",
				flexDirection: "column",
				gap: 10
			} }, react.default.createElement("div", { className: "onm-grid" }, react.default.createElement(Field, {
				label: "ONENAT 服务地址",
				children: react.default.createElement(TextInput, {
					value: baseUrl,
					mono: true,
				...
				onClick: save,
				disabled: busy
			}, busy ? "保存中…" : "保存")), message ? react.default.createElement(Banner, {
				kind: message.kind,
				text: message.text
			}) : null) : react.default.createElement("div", { className: "onm-muted" }, "ONENAT 资源面地址与派发默认值（超时、会话复用）。", baseUrl ? ` 当前：${baseUrl}` : " 当前未配置。"));
		}
		function ResourceCard() {
			const state = useAsync(async () => await consoleApi.resources(true) || {}, [], {});
			const endpoints = state.value?.endpoints || [];
			return react.default.createElement("div", { className: "onm-card" }, react.default.createElement("div", { className: "onm-card-head" }, react.default.createElement("h3", { className: "onm-card-title" }, "ONENAT 资源目录", react.default.createElement(Tag, { tone: "brand" }, String(endpoints.length))), react.default.createElement("div", { className: "onm-spacer" }), react.default.createElement(Btn, {
				size: "sm",
				onClick: state.reload,
				disabled: state.loading
			}, state.loading ? "刷新中…" : "↻ 刷新")), react.default.createElement("div", { className: "onm-muted" }, "在输入框用 @ 可以直接选中下列资源；条目里的公网入口是实时解析结果。"), state.error ? react.default.createElement("div", { style: { marginTop: 8 } }, react.default.createElement(Banner, {
				kind: "err",
				text: state.error
			})) : null, endpoints.length === 0 ? react.default.createElement("div", {
				className: "onm-muted",
				style: { marginTop: 8 }
			}, "暂无资源（检查 ONENAT 地址与 Key，或确认 ONENAT 客户端在线）。") : react.default.createElement("div", { className: "onm-list" }, endpoints.map((ep) => react.default.createElement("div", {
				key: ep.mappingId || ep.appId,
				className: "onm-item"
			}, react.default.createElement("div", { className: "onm-item-head" }, react.default.createElement("span", { className: "onm-item-title" }, "@" + (ep.appName || ep.note || ep.mappingId)), react.default.createElement(Tag, { tone: ep.online ? "ok" : "off" }, ep.online ? "在线" : "离线"), react.default.createElement(Tag, null, String(ep.kind || "").toUpperCase()), (ep.appSkills || []).length > 0 ? react.default.createElement(Tag, null, `${ep.appSkills.length} 技能`) : null), react.default.createElement("dl", { className: "onm-meta" }, react.default.createElement(MetaRow, {
				label: "入口",
				children: react.default.createElement("span", { className: "onm-mono" }, ep.kind === "ssh" ? `ssh -p ${ep.port} @${ep.host}` : ep.baseUrl || `${ep.proto}://${ep.host}:${ep.port || "?"}`)
			}), react.default.createElement(MetaRow, {
				label: "隧道",
				children: ep.tunnelName || "—"
			}))))));
		}
		const BLANK_SSH = {
			id: "",
		...
		function SshEditor(props) {
			...
			const [form, setForm] = react.default.useState(() => sshFormFrom(props.resource));
			const [message, setMessage] = react.default.useState(void 0);
			const [testResult, setTestResult] = react.default.useState(void 0);
			const [busy, setBusy] = react.default.useState(false);
			...
			const save = () => {
				...
				consoleApi.sshSave(payload).then(() => props.onSaved(isEdit ? `已更新「${form.name}」` : `已新增「${form.name}」`)).catch((err) => setMessage({
					kind: "err",
					text: String(err?.message || err)
				})).finally(() => setBusy(false));
			};
			const test = () => {
				setTesting(true);
				setTestResult("测试中…");
				consoleApi.sshTest(form.id).then((data) => {
					setTestResult(JSON.stringify(data, null, 2));
					setMessage({
						kind: data?.ok === false ? "err" : "ok",
						text: data?.ok === false ? "连通测试失败，详见下方结果。" : "连通测试通过。"
					});
				}).catch((err) => {
			...
			return react.default.createElement(Overlay, {
				...
				onClose: props.onClose,
				footer: [
					isEdit ? react.default.createElement(Btn, {
						key: "test",
						disabled: testing,
						onClick: test
					}, testing ? "测试中…" : "测试连通") : null,
					react.default.createElement("div", {
						key: "sp",
						className: "onm-spacer"
					}),
			...
			}, react.default.createElement("div", { className: "onm-hint" }, "本地 SSH 资源池是 ONENAT 之外的直连补充。@ 菜单里选中后，宿主会把该资源的连接方式（与凭证策略）注入本轮上下文；凭证只在本机存储，不回传远端。"), message ? react.default.createElement(Banner, {
				kind: message.kind,
				text: message.text
			}) : null, react.default.createElement("div", { className: "onm-grid" }, react.default.createElement(Field, {
				label: "名称（@ 菜单显示）",
				...
				children: react.default.createElement(TextInput, {
					...
					placeholder: isEdit && hasSavedCredential ? "不修改请留空" : "登录密码"
				})
			}) : react.default.createElement(Field, {
				label: "私钥（PEM 全文）",
			...
			}), testResult ? react.default.createElement("pre", { className: "onm-pre onm-mono" }, testResult) : null);
		}
		function SshCard(props) {
			...
			return react.default.createElement("div", { className: "onm-card" }, react.default.createElement("div", { className: "onm-card-head" }, react.default.createElement("h3", { className: "onm-card-title" }, "本地 SSH 资源池", react.default.createElement(Tag, { tone: "brand" }, String(resources.length))), react.default.createElement("div", { className: "onm-spacer" }), react.default.createElement(Btn, {
				size: "sm",
				variant: "primary",
				onClick: () => setEditing(null)
			}, "＋ 新增")), react.default.createElement("div", { className: "onm-muted" }, "补充 ONENAT 之外的直连主机。@ 菜单里选中后，凭证会随资源清单一并交给执行方。"), message ? react.default.createElement("div", { style: { marginTop: 8 } }, react.default.createElement(Banner, {
				kind: message.kind,
				text: message.text
			})) : null, resources.length === 0 ? react.default.createElement("div", {
				className: "onm-muted",
				style: { marginTop: 8 }
			}, "暂无本地 SSH 资源，点右上角「新增」添加一台。") : react.default.createElement("div", { className: "onm-list" }, resources.map((r) => react.default.createElement("div", {
				key: r.id,
				className: "onm-item"
			}, react.default.createElement("div", { className: "onm-item-head" }, react.default.createElement("span", { className: "onm-item-title" }, r.name), r.lastTestOk === void 0 ? null : react.default.createElement(Tag, { tone: r.lastTestOk ? "ok" : "off" }, r.lastTestOk ? "上次连通 ✓" : "上次失败 ✗"), r.hasPassword || r.hasPrivateKey ? react.default.createElement(Tag, null, r.hasPrivateKey ? "私钥" : "密码") : react.default.createElement(Tag, { tone: "warn" }, "未设凭证")), react.default.createElement("dl", { className: "onm-meta" }, react.default.createElement(MetaRow, {
				label: "地址",
				children: react.default.createElement("span", { className: "onm-mono" }, `${r.username}@${r.host}:${r.port}`)
			}), r.description ? react.default.createElement(MetaRow, {
				label: "说明",
				children: r.description
			}) : null), react.default.createElement("div", { className: "onm-actions" }, react.default.createElement(Btn, {
				size: "sm",
				onClick: () => {
			...
			}, "测试"), react.default.createElement(Btn, {
				size: "sm",
			...
			}, "删除"))))), detail ? react.default.createElement("pre", { className: "onm-pre onm-mono" }, detail) : null, editing !== void 0 ? react.default.createElement(SshEditor, {
				resource: editing,
				onClose: () => setEditing(void 0),
				onSaved: (text) => {
					setEditing(void 0);
		...
		*/
		function MentionStatusLine() {
			...
			const [busy, setBusy] = react.default.useState(false);
			const tone = status.state === "ready" ? "var(--dsw-alias-state-success-primary, #16a34a)" : status.state === "error" ? "var(--dsw-alias-state-error-primary, #dc2626)" : "var(--dsw-alias-state-warn-primary, #d97706)";
			...
			}), react.default.createElement("span", null, label), react.default.createElement(Btn, {
				size: "sm",
				variant: "ghost",
				onClick: refresh,
				disabled: busy
		...
		function MentionTester() {
			...
			return react.default.createElement("div", { className: "onm-card" }, react.default.createElement("h3", { className: "onm-card-title" }, "提及解析自检"), react.default.createElement("div", {
				className: "onm-muted",
				style: { marginTop: 6 }
			}, "验证一段文本会被解析成哪些实体（用于确认 @ 名称是否唯一可辨）。"), react.default.createElement("div", {
				className: "onm-row",
				style: { marginTop: 8 }
				...
				value: text,
				onChange: setText
			})), react.default.createElement(Btn, { onClick: () => consoleApi.parse(text).then((d) => setResult(JSON.stringify(d, null, 2))).catch((e) => setResult(String(e?.message || e))) }, "解析")), result ? react.default.createElement("pre", { className: "onm-pre onm-mono" }, result) : null);
		}
		function Section(props) {
			...
			return react.default.createElement("div", { className: "onm-root" }, react.default.createElement("div", { className: "onm-muted" }, "OneNat WorkBuddy @ —— 在 DSH 原生输入框用 @ 指定 ONENAT 上的子智能体与资源。选中即插入胶囊，发送后由宿主把「指认指令」与「资源清单」注入本轮上下文；子智能体派发走 onenat_agent 工具，远端会话按本会话长持复用。"), agents.error ? react.default.createElement(Banner, {
				kind: "err",
				text: agents.error
			}) : null, react.default.createElement(AgentCard, {
				agents: agentList,
				endpoints,
				onChanged: refreshAll
			}), react.default.createElement(ConnectionCard, {
				settings: settings.value,
				onSaved: refreshAll
			}), react.default.createElement(ResourceCard, null), react.default.createElement(SshCard, { onChanged: refreshAll }), react.default.createElement(MentionTester, null));
		}
		function apply(ctx) {
			const log = (msg) => {
				console.log(`[${NS}] ${msg}`);
