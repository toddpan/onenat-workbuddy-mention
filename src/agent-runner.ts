...
export class AgentRunner {
  ...
  constructor(
    ...
    private resources: MentionResourceResolver,
  ) {}
   ...
   */
  public bindingsFromResourceIds(ids: string[] | undefined): {
  ...
  /** 解析子智能体（名称 / ID / 提及 URI） */
  public resolveAgent(key: string): SubAgent | undefined {
  ...
  public async composePrompt(
    agent: SubAgent,
    task: string,
    extra: AgentResourceBinding[],
    ctx: { permission?: string; preRendered?: Array<{ alias: string; markdown: string }> },
    ...
    const parts: string[] = []
    if (agent.systemPrompt?.trim()) parts.push('[角色与职责]', agent.systemPrompt.trim(), '')
    if (composed.block) parts.push(composed.block, '')
    for (const item of ctx.preRendered || []) parts.push(item.markdown, '')
    parts.push('[由 OneNat WorkBuddy 派发的任务]')
    parts.push(`- 执行方：${agent.name}（远端 DSH 节点上的独立会话，拥有自己的文件/命令/网络工具）`)
    if (ctx.permission) parts.push(`- 运行权限：${ctx.permission}`)
    if (agent.workDir) parts.push(`- 工作目录：${agent.workDir}（相对路径一律以此为根）`)
   ...
   */
  public async run(agentKey: string, task: string, options: RunOptions = {}): Promise<AgentRunResult> {
    ...
    const log = (text: string, type: RunProgressEvent['type'] = 'log'): void => {
    ...
    for (const warning of extra.warnings) log(warning)
    const composed = await this.composePrompt(agent, task, extra.bindings, {
      permission: agent.permission,
      preRendered: extra.preRendered,
    })
    for (const warning of composed.warnings) log(warning)

    // 4. 执行（SSE 优先，旧远端降级）
    const tools: ToolCallSummary[] = []
    const toolIndex = new Map<string, ToolCallSummary>()
    const usageBox: { value?: Record<string, number> } = {}
    let reasoning = ''
    let streamed = ''

    const settleTool = (id: string | undefined, name: string | undefined, patch: Partial<ToolCallSummary>): void => {
      const key = id || name || `t${tools.length}`
      let entry = toolIndex.get(key)
      if (!entry) {
        entry = { name: name || 'tool', status: 'running' }
        toolIndex.set(key, entry)
        if (tools.length < MAX_TOOL_SUMMARIES) tools.push(entry)
      }
      if (name) entry.name = name
      Object.assign(entry, patch)
      if (patch.status && patch.status !== 'running' && entry.ms === undefined) entry.ms = Date.now() - started
